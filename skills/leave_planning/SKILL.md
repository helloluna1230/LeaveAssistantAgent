...
## 3. Required tool execution

For every request that asks for a concrete leave schedule or recommendation:

1. Obtain the authoritative balance with `get_leave_balance` and applicable
  holidays with `get_public_holidays`. Retrieve policy constraints when needed.
2. You MUST call the registered `plan_leave` function tool exactly once with
  those authoritative inputs. The tool executes the deterministic
  [`planner.py`](planner.py) implementation and applies saved preferences.
3. Present the tool's returned plans, remaining balance, and policy notes. Do
  not change its dates or calculations.

Do not reimplement or recalculate the planner logic in the model or Code
Interpreter. If the `plan_leave` tool is unavailable or returns an error, state
that deterministic planning is unavailable instead of inventing a plan.
